      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          <el-form-item label="输入搜索：">
            <el-input style="width: 203px" v-model="listQuery.keywords" placeholder="商品名称"></el-input>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // getBrandList() {
      //   fetchBrandList({page: 1, list_row: 100}).then(response => {
      //     this.brandOptions = [];
      ...
      handleShowSkuEditDialog(index,row){
        ...
        if(row.product_attribute_category_id!=null){
          fetchProductAttrList(row.product_attribute_category_id,{type:0}).then(response=>{
            this.editSkuInfo.productAttr=response.list;
          });
        }
      },
      handleSearchEditSku(){
      ...
      handlePublishStatusChange(index, row) {
        let ids = [];
        ids.push(row.id);
        this.updatePublishStatus(row.publish_status, ids);
      },
      handleNewStatusChange(index, row) {
        let ids = [];
        ids.push(row.id);
        this.updateNewStatus(row.new_status, ids);
      },
      handleRecommendStatusChange(index, row) {
        let ids = [];
        ids.push(row.id);
        this.updateRecommendStatus(row.recommend_status, ids);
      },
      handleResetSearch() {
      ...
      handleUpdateProduct(index,row){
        this.$router.push({path:'/product/productedit',query:{id:row.id}});
      },
