  ...
  export default {
    ...
    data() {
      return {
        ...
        ossUploadUrl:process.env.BASE_OOS_URL,
        minioUploadUrl:process.env.BASE_API+'/back/file/upload',
      }
    },
    methods: {
      checkAllSuccess() {
      ...
      handleSuccess(response, file) {
        ...
        for (let i = 0, len = objKeyArr.length; i < len; i++) {
          if (this.listObj[objKeyArr[i]].uid === uid) {
            ...
            if(!this.useOss){
              const resStr = response.code
              let resJsonObj=Decrypt(resStr);
              if (resJsonObj.status !== 1) {
                Message({
                  message: resJsonObj.msg,
                  type: 'error',
                  duration: 3 * 1000
                })
              }else{
                url =  resJsonObj.data.url;
              }
            }
